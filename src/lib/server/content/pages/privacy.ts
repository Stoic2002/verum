 ...
 * no identifier, a language cookie only when the reader picks one, and ad and
...
**Pilihan bahasa.** Kalau Anda memilih bahasa lewat tombol EN/ID, pilihan itu disimpan di cookie \`verum-locale\` selama setahun, supaya kunjungan berikutnya ke verumdaily.com langsung membuka bahasa yang sama. Cookie ini tidak berisi data lain.
...
**Language choice.** If you pick a language with the EN/ID switch, that choice is kept in a \`verum-locale\` cookie for a year, so your next visit to verumdaily.com opens in the same language. The cookie holds nothing else.
